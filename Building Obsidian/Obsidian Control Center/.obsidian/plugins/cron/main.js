...
var Job = class {
  ...
  jobIntervalPassed() {
    ...
    if (!lastRun)
      return true;
    const prevRun = window.moment((0, import_cron_parser.parseExpression)(this.frequency).prev().toDate());
...
// src/settings.ts
var CronSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Cron Interval").setDesc("The interval the cron will run in minutes").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("Run cron on startup").setDesc("Do a cron run on startup instead of waiting for the first interval to pass").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.runOnStartup).onChange(async (value) => {
        this.plugin.settings.runOnStartup = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Enable Obsidian Sync Checker").setDesc("Whether or not to wait for Obsidian sync before running any CRONs globally.").addToggle(
    ...
    new import_obsidian2.Setting(containerEl).setName("Enable Obsidian on Mobile").setDesc("Whether or not to load jobs at all on Mobile devices. If disabled even jobs with mobile enabled will not run.").addToggle(
    ...
    new import_obsidian2.Setting(containerEl).setName("Cron Jobs").setDesc(desc);
    this.addCommandSearch();
  }
  addCommandSearch() {
    this.plugin.settings.crons.forEach((cronjob, index) => {
      const jobSetting = new import_obsidian2.Setting(this.containerEl).addText(
...
var Cron = class extends import_obsidian3.Plugin {
  async onload() {
